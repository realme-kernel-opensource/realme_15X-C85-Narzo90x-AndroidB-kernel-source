...
static kal_uint8 ct_read_data[CROSSTALK_BUF_SIZE + 2];

void gc32e2_rado_read_crosstalk_data(void)
{
	kal_uint16 i = 0;
...
//	ct_flag = 1;
	if (ct_flag == OTP_FLAG_VALID) {
		ct_read_data[0] = 0x5c;
		ct_read_data[1] = 0x05;
		for (i = 2; i < CROSSTALK_BUF_SIZE + 2; i++)
...
void gc32e2_rado_copy_crosstalk_data(kal_uint8 *ct_data)
{
	LOG_INF("start copy crosstalk_data!\n");
	memcpy(ct_data, (void *)&ct_read_data[0], GC_SXTC_DATA_SIZE + 2);
	memcpy(ct_data + GC_SXTC_DATA_SIZE + 2, (void *)&ct_read_data[2 + GC_SXTC_DATA_SIZE + FLAG_AND_CHECK_SUM_OF_SXTC_SIZE + SUNNY_RESERVED_SIZE], GC_PDXTC_DATA_SIZE);
}



void gc32e2_rado_otp_identify(void)
{
	LOG_INF("start hyper otp_identify");
	gc32e2_rado_read_crosstalk_data();
