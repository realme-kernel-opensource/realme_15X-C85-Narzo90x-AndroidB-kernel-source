...
#include "imx480mipiraw_mumbaih_Sensor.h"
...
static BYTE imx480_mumbaih_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX480_SENSOR_ID_MUMBAIH,
...
static kal_uint16 read_imx480_mumbaih_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };
...
static void read_imx480_mumbaih_module_data()
{
...
// sensorid : [30 ~ 33]
	...
	memset(imx480_mumbaih_common_data, 0, sizeof(imx480_mumbaih_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		imx480_mumbaih_common_data[8 + idx] = read_imx480_mumbaih_eeprom_module(sn_starAddr + idx);
	}
	//vcm
	...
	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
		printk("cam data, idx: %02x %02x %02x %02x %d\n",
		       imx480_mumbaih_common_data[idx],
		       imx480_mumbaih_common_data[idx + 1],
		       imx480_mumbaih_common_data[idx + 2],
		       imx480_mumbaih_common_data[idx + 3],
			   idx);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        *sensor_id = return_sensor_id();
        if (*sensor_id == 0x0471) {
            *sensor_id = imgsensor_info.sensor_id;
            LOG_INF("IMX480 get_imgsensor_id success: 0x%x\n", *sensor_id);
            read_imx480_mumbaih_module_data();
            return ERROR_NONE;
        }
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        LOG_INF("IMX480 get_imgsensor_id failed: 0x%x\n", *sensor_id);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE:
        *(MINT32 *)(signed long)(*(feature_data + 1)) = 2185334;
        break;
    ...
    case SENSOR_FEATURE_GET_EEPROM_COMDATA:
        memcpy(feature_return_para_32, imx480_mumbaih_common_data, CAMERA_EEPPROM_COMDATA_LENGTH);
...
UINT32 IMX480_MIPI_RAW_MUMBAIH_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
