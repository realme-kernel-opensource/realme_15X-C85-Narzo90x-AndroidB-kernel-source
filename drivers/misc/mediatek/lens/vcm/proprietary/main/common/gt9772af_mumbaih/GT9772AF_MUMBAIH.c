...
/*
 * GT9772AF_MUMBAIH voice coil motor driver
 *
 *
 */
...
#define AF_DRVNAME "GT9772AF_MUMBAIH_DRV"
...
static int initAF(void)
{
    ...
    if (*g_pAF_Opened == 1) {
        ...
        //ret = s4AF_WriteReg(0, 0x02, 0x00); //advance mode
        mdelay(5);
        INIT_ret = s4AF_WriteReg(0, 0x0B, 0x50); //SDC enable and SDC_C set
        EN_AAC_ret = s4AF_WriteReg(0, 0x02, 0x02); //PRESC[1:0]=01
        AAC2_ret = s4AF_WriteReg(0, 0x06, 0xC0);
        TVIB_ret = s4AF_WriteReg(0, 0x07, 0x7F); //SDTC[6:0]=1001001
        LOG_INF("INIT_ret: %x EN_AAC_ret: %x AAC2_ret: %x TVIB_ret: %x\n", INIT_ret, EN_AAC_ret, AAC2_ret, TVIB_ret);
...
/* ////////////////////////////////////////////////////////////// */
long GT9772AF_MUMBAIH_Ioctl(struct file *a_pstFile, unsigned int a_u4Command,
...
int GT9772AF_MUMBAIH_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
    int Ret = 0;
...
int GT9772AF_MUMBAIH_PowerDown(struct i2c_client *pstAF_I2Cclient,
        int *pAF_Opened)
{
...
int GT9772AF_MUMBAIH_SetI2Cclient(struct i2c_client *pstAF_I2Cclient,
...
int GT9772AF_MUMBAIH_GetFileName(unsigned char *pFileName)
{
    if (pFileName == NULL)
        return 0;
    ...
    *FileString = '\0';
    ...
    if (strlen(FileString) >= AF_MOTOR_NAME)
        return 0; // Not enough space in pFileName

    strncpy(pFileName, FileString, AF_MOTOR_NAME - 1);
    pFileName[AF_MOTOR_NAME - 1] = '\0'; // Ensure null termination
    LOG_INF("FileName : %s\n", pFileName);
    #else
    pFileName[0] = '\0';
    #endif
