...
static struct device_node *oplus_get_node_by_type(struct device_node *father_node)
{
	...
	bat_type = oplus_get_battype_str_cmdline();
	if (bat_type) {
		sub_node = of_get_child_by_name(father_node, bat_type);
		if (sub_node) {
...
static int oplus_batid_get_cmdline(void)
{
	...
	if (np) {
		...
		if (!bootparams || ret < 0) {
			bm_err("failed to get bootargs property");
			return -1;
		}
